...
|---|---|---|
| `team.json` | 24 | One record per person on Who We Are. Fields: `name`, `group` (Leadership, Research Partners, Education Fellows, Technology and Design Partners — in that order), `role`, `affiliation` and `location` (null except for fellows and some partners), `photo` (a repository-relative path, or **null for the 6 people with no usable photograph yet** — model the image as optional and render the gray placeholder), `bioUrl` (the person's bio page, or null — four leaders have one), `links` (zero or more of LinkedIn / Website). |
