...
|---|---|---|
| `team.json` | 29 | One record per person on Who We Are. Fields: `name`, `group` (Leadership, Research Partners, Education Fellows, Technology and Design Partners — in that order), `role` (**null for the 5 people added from the website info form before their titles were known** — model it as optional and render nothing, not an empty line), `affiliation` and `location` (null except for fellows and some partners), `photo` (a repository-relative path, or **null for the 5 people with no usable photograph yet** — model the image as optional and render the gray placeholder), `bioUrl` (the person's bio page, or null — four leaders have one), `links` (zero or more of LinkedIn / Website). |
...
The four leadership bios are on the site, one page each at the `bioUrl` above. Their
prose lives in `source-material/bios/<slug>.md` and nowhere else — deliberately not in
this export, so there is only one copy to keep current. See
[`../sections/leadership.md`](../sections/leadership.md). Bios for the wider team exist
in our tracking sheet; ask us for it if the rebuild wants them.
