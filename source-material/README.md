...
| `image-sources/` | The originals `tools/encode-images.mjs` reads: `team/`, `images/`, `icons/` (the three home-page illustration plates), `blocks/` (the six wooden-block cut-outs) and `unused/`. `unused-diagrams/` holds the co-design cycle SVGs the R&D wheel was drawn from. |
